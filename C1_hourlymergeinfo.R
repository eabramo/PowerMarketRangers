...
dfX2 <- dfX %>%
  ...
  summarize(lmp = sum(lmp*load_MW),
            load_MW = sum(load_MW),
            n = n()
            )
  # sum value = lmp ($/MW)* (MW) = total value for each one hour interval
  # sum units, load_MW for each one hour interval
...
dfX2$houravglmp <- (dfX2$lmp)/dfX2$load_MW
...
write.csv(dfXW, file = 'hourly_lmp_load_weather.csv')
